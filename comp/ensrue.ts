export async function addSlide(preview: Preview) {
  ...
  // Delete the 'duration' and 'time' property values
  // to make each element (Slide-1, Slide-2, etc.) autosize on the timeline
  ...
  const lastSlideIndex = source.elements.findLastIndex(
    (element: any) => element.name?.startsWith('Slide-'),
  );
